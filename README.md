...
> Most visuals below are **hand-drawn SVG mockups** — they show shape +
> intent, not pixel-exact screenshots. The chat-UI section uses real
> headless-Chrome captures generated by
> [`scripts/bench_chrome_check.py`](scripts/bench_chrome_check.py)
> (re-run it to refresh those). The Qt-app SVGs (admin dashboard,
> metrics, diagnostics, login dialog, setup wizard, launcher console)
> are still the original mockups — the underlying windows haven't
> changed structurally since they were drawn. Updates to the chat web
> UI captured here: tier dropdown grouped by category (Reasoning /
> Coding optgroups), module-aggregated 🔧 Tools popover (~150 module
> rows instead of 620 individual methods), `[LAI-XXX-NNN]` error-code
> badges, and 💾 / ⚠️ tier badges for MoE expert offload + NVMe spillover.
...
<p align="center">
  <img src="docs/images/chat-tools-popover-collapsed.png" alt="Chat UI with the Tools popover open in its default-collapsed state — two top-level tier categories (Network-only Tools / Host & System Access) with master toggles" width="900"/>
  <br/>
  <em>Tools popover opens collapsed: two tier categories, click to drill in.</em>
</p>

<p align="center">
  <img src="docs/images/chat-tools-popover-drilled.png" alt="Chat UI with the Tools popover drilled into Network-only Tools → Research & Academia → Cross-disciplinary, showing module-aggregated rows: Crossref, Doaj, Openalex, Semantic Scholar, Unpaywall, Zenodo (each with a single master toggle)" width="900"/>
  <br/>
  <em>Drilled: module-aggregated rows under each subgroup. One toggle per
  module enables every method beneath it (per-method granularity stays
  in <code>/admin/tools</code>). Module-name display is friendly —
  <code>crossref</code> shows as "Academic citations (Crossref)",
  <code>filesystem</code> as "Files & folders", etc.</em>
</p>
